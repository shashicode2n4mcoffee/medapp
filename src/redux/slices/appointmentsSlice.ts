...
import { appointmentService, AppointmentListResponse, Appointment, GetAppointmentsParams } from '../../api/appointmentService';
...
>(
  ...
  async (params, { getState, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'An unknown error occurred while fetching appointments');
    }
  }
);
...
const appointmentsSlice = createSlice({
  ...
  initialState,
  reducers: {
    updateParams: (state, action: PayloadAction<Partial<GetAppointmentsParams>>) => {
      state.currentParams = {
        ...state.currentParams,
        ...action.payload
      };
...
export const { updateParams, clearAppointments, clearError } = appointmentsSlice.actions;
